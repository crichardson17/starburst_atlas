...
for i in range(numFiles):
	for file in os.listdir('.'):
		if file.endswith("padova_cont_5_{:d}_supersolar_highres.grd".format(i+1)):
			gridFiles[i] = file
			#keep track of all the files you'll be importing by printing 
			print file
		if file.endswith("padova_cont_5_{:d}_supersolar_highres.emissionlines_abs.txt".format(i+1)):			
...
def add_sub_plot(sub_num, elinesplot):
	...
	contour = plt.contour(x_axis, y_axis, z_line, levels, colors='c', linestyles = 'dashed', extent=extent) #teal contours, dashed
	contour2 = plt.contour(x_axis, y_axis, z_line, levels2, colors='k', linewidths=1.5) #black contours, solid
...
levels2 = arange(10**-2,10**2, 1) #black levels 
# ---------------------------------------------------
