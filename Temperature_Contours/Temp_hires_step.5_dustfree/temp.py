...
# ---------------------------------------------------
#for grid
phi_values = grid[1:len(datatemp)+1,6]
hdens_values = grid[1:len(datatemp)+1,7]
...
# Interpolate
print "starting interpolation"
xion, yion = linspace(xio.min(), xio.max(), (xio.max()-xio.min())*2+2), linspace(yio.min(), yio.max(), (yio.max()-yio.min())*2+2) 
...
plt.suptitle("Temperature Contours- Padova Continuous Age 5 Myr Dustfree", fontsize=18)
