...
def add_sub_plot(sub_num):
	...
	zi = rbf(x, y)
	#contour1 = plt.contour(x_axis, y_axis, z, levels, colors='k', origin='lower', extent=extent) #teal contours, dashed

	#our contours. Teal for .2 dex and black for 1 dex
	contourmap = plt.imshow(z, cmap='Reds', extent= extent, aspect = "auto",origin='lower', vmin=2, vmax =7)

	#contour = plt.contour(x_axis,y_axis,z, levels, colors='c', linestyles = 'dashed')
	#contour2 = plt.contour(x,y,z, levels2, colors='k', linewidths=1.5)
...
#zi = np.reshape(z,(Ny,Ny))




plt.subplots_adjust(wspace=0, hspace=0) #remove space between plots
levels = arange(10**-2,10, 1) #black levels
